 ...
 *  user can overwrite. Order matters: suggestHalfLifeHours takes the first key found in the name, so a
 *  more specific variant must precede the bare name it contains ('cjc-1295 dac' before 'cjc-1295').
 *  Brand names map to their compound. Deliberately absent: BPC-157, TB-500, GHK-Cu, KPV, Melanotan II,
 *  AOD-9604, hexarelin, GHRP-2 and friends — no dependable human PK to quote, so those compounds get no
 *  suggestion and the field stays the user's own to fill in. Adding keys? Bump HALF_LIFE_TABLE_VERSION. */
export const STANDARD_HALF_LIVES_HOURS: Record<string, number> = {
	'cjc-1295 dac': 168, // with DAC: ~6-8 days
	'cjc-1295 with dac': 168,
	'cjc-1295 w/ dac': 168,
	...
	setmelanotide: 11,
	imcivree: 11,
	mecasermin: 5.8, // rhIGF-1 (Increlex) — not IGF-1 LR3, which has no human PK
	increlex: 5.8,
	bremelanotide: 2.7,
	'pt-141': 2.7, // bremelanotide under its common name
	vyleesi: 2.7,
	'ghrp-6': 2.5, // IV, terminal phase
	ghrp6: 2.5,
	exenatide: 2.4,
	byetta: 2.4,
	ipamorelin: 2,
	'thymosin alpha': 2, // thymalfasin (Zadaxin), SC
	thymalfasin: 2,
	zadaxin: 2,
	'ta-1': 2,
	ta1: 2,
	...
	'melanotan 1': 1.25,
	'melanotan-1': 1.25,
	'melanotan i': 1.25, // the name boundary check keeps this off "Melanotan II"
	'melanotan-i': 1.25,
	'mt-1': 1.25,
	'mt-i': 1.25,
	teriparatide: 1, // SC
...
/** True when `key` occurs in `name` (both lowercase) and isn't glued to a longer word/number: the characters
 *  around the match must not continue the key's own first/last character class. So 'melanotan i' misses
 *  "melanotan ii", 'ta1' misses "beta1" and 'melanotan 1' misses "melanotan 10mg", while
 *  "retatrutide 10mg/ml" and "semaglutide5mg" still match. */
...
/** Case-insensitive match against STANDARD_HALF_LIVES_HOURS — tolerant of a compound name that isn't an
 *  exact key (e.g. "Retatrutide 10mg/mL"). Null when nothing matches. */
