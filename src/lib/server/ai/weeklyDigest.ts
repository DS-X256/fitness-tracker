...
export async function generateWeeklyDigest(userId: number, today: string): Promise<Result> {
	...
	const digest = await callAndSave(userId, weekStart, today, create);
	if (!digest) return { error: 'AI is not configured or the request failed. Try again later.' };
...
export async function regenerateWeeklyDigest(userId: number, today: string): Promise<Result> {
	...
	const digest = await callAndSave(userId, weekStart, today, regenerate);
	if (!digest) return { error: 'AI is not configured or the request failed. Try again later.' };
	return { digest, fromCache: false };
}
...
async function callAndSave(
	...
	return save(userId, weekStart, result.text.trim(), result.model);
}
